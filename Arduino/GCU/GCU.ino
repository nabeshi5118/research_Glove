...
const uint16_t device_frequency = 100; //データを送る頻度
const uint16_t calibration_duration = 10000;

// Sensor Numbers
//Vscodeと合わせる
constexpr unsigned char sensors_rows_num = 2;
constexpr unsigned char sensors_columns_num = 2;
...
const bool IMU_flag = GCU_FLAG_ON; //ここで加速度センサーのフラグを ONにしている
...
//
const bool IMU_chip = GCU_BMI270_BMM150; //GCUのモデルを変更する
...
const char* SSID       = "Buffalo-G-D94E";
const char* password   = "c5bt88k45hrh4";

// UDP broadcast
const char* SeverIP = "192.168.11.14";
...
const int analogReadIO[]={1,2};
const int SelectIO[]={18,19};
...
float distance_x = 0.0, distance_y = 0.0, distance_z = 0.0;
unsigned long last_update_time = 0;
const float G_TO_MS2 = 9.80665; // Gを m/s^2 に変換する定数
...
void setup() {
  ...
  data_receiver.attach_ms(1000/device_frequency, on_timer); //1秒に10個

}


void loop() {
  // GCU.ino (loop関数の下などに追加)
  ...
  if (IMU_flag){
    if (IMU_chip){
      ...
      Serial.print(BMI270_BMM150_accel_x);
      Serial.print("  Y: ");
      Serial.print(BMI270_BMM150_accel_y);
      Serial.print("  Z: ");
      Serial.println(BMI270_BMM150_accel_z);
